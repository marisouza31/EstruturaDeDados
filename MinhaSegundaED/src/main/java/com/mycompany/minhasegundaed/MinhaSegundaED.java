...
public class MinhaSegundaED {
    ...
    private int getIndice(String nome) {
        if (nome == null || nome.isEmpty() || !nomeValido(nome)) {
            throw new IllegalArgumentException("Nome inválido: '" + nome + "'. Não deve conter caracteres especiais ou números.");
        }
    ...
    public void remove(String nome) {
        ...
        if (posicaoOcupada(posicao) && nomes[posicao].equals(nome)) {
            ...
            System.out.println("Nome não encontrado na estrutura.");
        }
    }
    ...
    public void exibirEstrutura() {
        System.out.println("Estrutura de nomes armazenados:");
        for (int i = 0; i < 26; i++) {
            if (nomes[i] != null) {
    ...
    public static void main(String[] args) {
        ...
        estrutura.adiciona("João"); // Permitido pois aceita acentos
        estrutura.adiciona("Karol");
        estrutura.adiciona("Luan");

        // Teste com nome inválido
        try {
            estrutura.adiciona("Rebeca!"); // Deve lançar exceção e mostrar o nome inválido
        } catch (IllegalArgumentException e) {
            System.out.println(e.getMessage());
        }

        try {
            estrutura.adiciona("M4ria"); // Deve lançar exceção e mostrar o nome inválido
        } catch (IllegalArgumentException e) {
            System.out.println(e.getMessage());
        }

        estrutura.exibirEstrutura();

        System.out.println("A estrutura contém 'Davi'? " + estrutura.contem("Davi"));
        System.out.println("A estrutura contém 'Natalia'? " + estrutura.contem("Natalia"));

        estrutura.remove("Carlos");
